      ...
      {[...Array(2)].map((_, setIdx) => (
        <div key={setIdx} className="flex gap-6 pr-6">
          {items.map((partner, idx) => (
            <div key={`${setIdx}-${idx}`} className="bg-white border border-gray-100 rounded-[2rem] p-4 w-32 h-32 flex-shrink-0 flex items-center justify-center grayscale hover:grayscale-0 transition-all duration-300 cursor-default shadow-sm hover:shadow-lg relative overflow-hidden group">
              {partner.logo ? (
                <div className="relative w-20 h-20 transition-transform duration-300 group-hover:scale-105">
                  <Image src={partner.logo} alt={partner.name} fill className="object-contain" />
                </div>
              ) : (
                <span className="font-bold text-[#828282] text-xs text-center transition-colors duration-300 group-hover:text-dark-900 leading-tight">{partner.name}</span>
              )}
            </div>
          ))}
        </div>
    ...
    <section id="partners" className="py-24 bg-white text-dark-900 overflow-hidden">
      <div className="container mx-auto px-6 md:px-12 mb-16">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Backed By 50+ Banks & NBFCs</h2>
        </div>
      </div>
